...
>   改成同一动态表达式。低优先。
>
> **GSC 侧状态（2026-09-15 完成）**
> - ✅ **「更改地址」已通过并确认**：必需项（来自首页的 301 重定向 / 验证两个网站）全绿，
>   目标 `gridpaw.com`，GSC 显示「**迁移进行中**」（持续 180 天）
> - ❌ **不撤 sitemap**：实测该属性页面**根本没有删除入口**；而且保留更有利——
>   Google 定期重读它、抓这些 URL、拿到 301，**sitemap 正是它发现迁移的通道**，
>   撤掉只会拖慢。先前「撤掉省抓取配额」的说法是错的（因果反了）
> - ⚠️ **别用「移除」(Removals) 工具**：它是临时从搜索结果摘 URL，用它等于跳过 301 的
>   信号传递——那些页面攒的权重会**丢掉**而不是交给新站
> - 纪律：301 至少留 **180 天**（有流量继续延长）；旧域名建议**续费满 1 年**防被他人买走
>
> **复验命令**
> ```bash
